...
const ProfileDetails = () => {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setImagePreview(
        profile.photo
          ? `https://api.saishraddhajewellers.com${profile.photo}`
          : Avatar
      );
    }
  }, [profile]);
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 flex flex-col items-center pt-12 px-4 sm:px-6 md:px-8 lg:px-12">
      <div className="max-w-4xl w-full bg-white rounded-lg p-6 sm:p-8 lg:p-12">
        <h1 className="text-2xl sm:text-3xl font-semibold text-primary-color mb-6 sm:mb-8">
          Account Overview
        </h1>
          ...
          {isEditing ? (
            <div>
              <div className="flex flex-col md:flex-row items-start gap-6">
                <div className="flex flex-col items-center md:w-1/2">
                  <label className="text-gray-700 mb-2 text-lg">
                    Profile Image
                  </label>
                  <input
                    type="file"
                  ...
                  {isLoading ? (
                    ...
                    <img
                      ...
                      alt="Profile Preview"
                      className="mt-4 w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-full object-cover"
                    />
                  )}
                </div>
            ...
            <div className="flex flex-col-reverse md:flex-row gap-6">
              <div className="flex flex-col space-y-4 md:w-1/2">
                ...
                {profile.pan_no && (
                  <p className="text-gray-700 text-lg">
                    <strong>PAN Card:</strong> {profile.pan_no}
                  </p>
                )}
                {profile.address && (
                  <p className="text-gray-700 text-lg">
                ...
                {profile.gst_no && (
                  <p className="text-gray-700 text-lg">
                    <strong>GST:</strong> {profile.gst_no}
                  </p>
                )}
              </div>
